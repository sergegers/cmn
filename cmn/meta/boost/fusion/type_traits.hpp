...
template <typename I> using deref_t = deref<I>::type;
...
template <typename Iterator, std::ptrdiff_t N_> using advance_c_t = advance_c<Iterator, N_>::type;

template <typename Iterator> using next_t = next<Iterator>::type;
...
template <c::fus_sequence Seq> using begin_t = begin<Seq>::type;
template <c::fus_sequence Seq>using end_t = end<Seq>::type;
