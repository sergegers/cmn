...
////////////////////////////////////////////////////////////////////////////////////////////////
//
// struct record_t { m_val, m_str, m_wstr };
// group_t = std::array<record_t>;
// groups_t = std::tuple<group_t>;
//
// simple enum: 1 group with n records
// bitfield: n groups with 1 record in each
// combo: n groups with m records in each
//
////////////////////////////////////////////////////////////////////////////////

template <c::enum_ Enum> constexpr auto get_name(Enum)->std::string_view = delete;
template <c::enum_ Enum> constexpr auto get_wname(Enum)->std::wstring_view = delete;
...
struct traits<Enum, true>
{
    ...
    // that enum definition (macros family CHT_PP_DECLARE_ENUM_...) is not included
    ...
    static constexpr auto masks = utils_type::calc_masks(groups);

    static constexpr Enum begin = utils_type::min_value(groups);
    static constexpr Enum last = utils_type::max_value(groups);
    ...
    {
        static_assert(std::tuple_size_v<decltype(groups)> > 0, "Must be at least one group!");

        if constexpr (masks[Idx_] == Mask_)
            return std::get<Idx_>(groups);
        else
...
struct traits<Enum, false>
{
    ...
    static constexpr kind_t kind = kind_t::simple;
};
...
template <typename Char, c::ext_enum Enum>
...
template <c::ext_any_enum Enum>
constexpr Enum begin_v = traits<Enum>::begin;

template <c::ext_any_enum Enum>
constexpr Enum last_v = traits<Enum>::last;

template <c::ext_any_enum Enum>
constexpr Enum end_v = traits<Enum>::end;
