#pragma once

#include <utility>

#include <boost/fusion/adapted/std_tuple.hpp>

#include "tuple.h"
#include "tuple_view.h"

namespace cmn::va
{

////////////////////////////////////////////////////////////////////////////////
//
// Structure bindings support
//
///////////////////////////////////////////////////////////////////////////////
template <std::size_t Idx_, typename... EArgs>
constexpr auto get(tuple_view<EArgs...> const &t) noexcept -> decltype(auto)
{
    return t.template get<Idx_>();
}

// NOTE: function returns value, not reference
template <typename T, typename... EArgs>
constexpr auto get(tuple_view<EArgs...> const &t) noexcept -> decltype(auto)
{
    return t.template get<T>();
}

template <std::size_t Idx_, typename... EArgs>
constexpr auto get(tuple<EArgs...> const &t) noexcept -> decltype(auto)
{
    return t.template get<Idx_>();
}

template <typename T, typename... EArgs>
constexpr auto get(tuple<EArgs...> const &t) noexcept -> decltype(auto)
{
    return t.template get<T>();
}

} 

////////////////////////////////////////////////////////////////////////////////
//
// There is no appropriate extension point, so we are intrude into std
// namespace. So, there is no guarantee that all std tuple functions & metafunctions works.
// Sorry about that.
// NOTE: use boost::fustion::invoke() instead of std::apply()
//
///////////////////////////////////////////////////////////////////////////////
namespace std
{

////////////////////////////////////////////////////////////////////////////////
//
// adapt va_tuple_view
//
////////////////////////////////////////////////////////////////////////////////
template <typename... EArgs>
struct tuple_size<cmn::va::tuple_view<EArgs...>> :
    integral_constant<size_t, cmn::va::tuple_view<EArgs...>::size()>
{};

template <std::size_t Idx_, typename... EArgs>
struct tuple_element < Idx_, cmn::va::tuple_view<EArgs...>>
{
    using type = cmn::va::tuple_element_t<Idx_, EArgs...>;
};

template <std::size_t Idx_, typename... EArgs>
constexpr auto get(cmn::va::tuple_view<EArgs...> const &t) noexcept -> decltype(auto)
{
    return t.template get<Idx_>();
}

template <typename T, typename... EArgs>
constexpr auto get(cmn::va::tuple_view<EArgs...> const &t) noexcept -> decltype(auto)
{
    return t.template get<T>();
}

// tuple_cut support
//template <typename... EArgs>
//struct _View_as_tuple<cmn::va::tuple_view<EArgs...>>
//{
//    using type = cmn::va::tuple_args_t<EArgs...>;
//};

////////////////////////////////////////////////////////////////////////////////
//
// adapt va_tuple
//
////////////////////////////////////////////////////////////////////////////////
template <typename... EArgs>
struct tuple_size<cmn::va::tuple<EArgs...>> :
    integral_constant<size_t, cmn::va::tuple<EArgs...>::size()>
{};

template <std::size_t Idx_, typename... EArgs>
struct tuple_element < Idx_, cmn::va::tuple<EArgs...>>
{
    using type = cmn::va::tuple_element_t<Idx_, EArgs...>;
};

template <std::size_t Idx_, typename... EArgs>
constexpr auto get(cmn::va::tuple<EArgs...> const &t) noexcept -> decltype(auto)
{
    return t.template get<Idx_>();
}

template <typename T, typename... EArgs>
constexpr auto get(cmn::va::tuple<EArgs...> const &t) noexcept -> decltype(auto)
{
    return t.template get<T>();
}

// tuple_cut support
//template <typename... EArgs>
//struct _View_as_tuple<cmn::va::tuple<EArgs...>>
//{
//    using type = cmn::va::tuple_args_t<EArgs...>;
//};

}   

////////////////////////////////////////////////////////////////////////////////
//
// adapt to FUSION and MPL algorithms
// NOTE: dispatch operations except at<> through std_tuple_tag
// at<> operation returns by va_tuple element by value because there is a 
// conversion float -> double is performed during float element access. So
// the va_tuple/va_tuple_view are 'read-only' ones.
//
////////////////////////////////////////////////////////////////////////////////
namespace boost
{

namespace fusion
{

using va_tuple_tag = cmn::va::tuple_tag;
struct fusion_sequence_tag;

namespace traits
{

template <typename... EArgs>
struct tag_of<cmn::va::tuple_view<EArgs...>>
{
    using type = va_tuple_tag;
};

template <typename... EArgs>
struct tag_of<cmn::va::tuple<EArgs...>>
{
    using type = va_tuple_tag;
};

}   

namespace extension
{
	
template <> struct is_view_impl<va_tuple_tag> : is_view_impl<std_tuple_tag> {};
template <> struct is_sequence_impl<va_tuple_tag> : is_sequence_impl<std_tuple_tag> {};
template <> struct category_of_impl<va_tuple_tag>: category_of_impl<std_tuple_tag> {};
template <> struct begin_impl<va_tuple_tag> : begin_impl<std_tuple_tag> {};
template <> struct end_impl<va_tuple_tag> : end_impl<std_tuple_tag> {};
template <> struct size_impl<va_tuple_tag> : size_impl<std_tuple_tag> {};

// return by value
template <>
struct at_impl<va_tuple_tag>
{
    template <typename Sequence, typename N>
    struct apply
    {
        using seq_type = std::remove_const_t<Sequence>;
        using element = std::tuple_element_t<N::value, seq_type>;
        using type = element;

        static auto call(Sequence &seq) noexcept -> type
        {
            return std::get<N::value>(seq);
        }
    };
};

template <> struct value_at_impl<va_tuple_tag> : value_at_impl<std_tuple_tag> {};
template <> struct convert_impl<va_tuple_tag> : convert_impl<std_tuple_tag> {};

}

namespace detail
{

// FUSION clear operation returns va_tuple type. If it will need to distinguish
// va_tuple and va_tuple_view types in FUSION clear algorithm the new 
// va_tuple_view_tag will be required.
template <>
struct clear<va_tuple_tag> : std::type_identity<cmn::va::tuple<>> {};

}

}   

namespace mpl
{

template <typename>
struct sequence_tag;

template <typename... EArgs>
struct sequence_tag<cmn::va::tuple_view<EArgs...>>
{
    using type = fusion::fusion_sequence_tag;
};

template <typename... EArgs>
struct sequence_tag<cmn::va::tuple_view<EArgs...> const>
{
    using type = fusion::fusion_sequence_tag;
};

template <typename... EArgs>
struct sequence_tag<cmn::va::tuple<EArgs...>>
{
    using type = fusion::fusion_sequence_tag;
};

template <typename... EArgs>
struct sequence_tag<cmn::va::tuple<EArgs...> const>
{
    using type = fusion::fusion_sequence_tag;
};

}   

}   

